...
if __name__ == '__main__':
    ...
    parser.add_argument("-mix_epoch", type=int, default=50)
    parser.add_argument("-sample_window", type=int, default=144)
    args = parser.parse_args()

    TRANSFORMER_REGRESSOR_CONFIG["seq_len"] = args.sample_window
    LINEAR_NEURAL_REGRESSOR_CONFIG["seq_len"] = args.sample_window
    CONVOLUTION_REGRESSOR_CONFIG["seq_len"] = args.sample_window
    ...
    for iexpert in range(NUM_EXPERTS):
        ...
        if (os.path.exists(os.path.join(args.work_dir, 'model', fname_model + f'_expert_{iexpert}.pt'))):
            continue
        ...
        for iepoch in range(EXPERT_TRAIN_EPOCHS[iexpert]):
            for iter, data in enumerate(iterator):
                ...
                print(f'Expert {iexpert} - Training loss in Epoch {iepoch}: {train_loss} hours.', flush=True)
        ...
        model_checkpoint_path = os.path.join(args.work_dir, 'model', fname_model + f'_expert_{iexpert}.pt')
    ...
    for iexpert in range(NUM_EXPERTS):
        checkpoint_path = os.path.join(args.work_dir, 'model', fname_model + f'_expert_{iexpert}.pt')
    ...
    for iepoch in range(MOE_TRAIN_EPOCHS):
        for iter, data in enumerate(iterator):
            data_input, data_target = data
            train_loss = moe_predictor.train_batch(data_input, data_target)
            # print(f'Mixture of Expert training... epoch {iepoch}:iter {iter}')
            train_loss = round(train_loss, 2)
            print(f'Mixture of Expert - Training loss in Epoch {iepoch}: {train_loss} hours.', flush=True)
            niters += 1
    ...
    model_checkpoint_path = os.path.join(args.work_dir, 'model', f'moe_{DATA_FILE_NAME}.pt')
